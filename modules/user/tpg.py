...
def tpg(favicon, wikiname, username, gender, registertime, contributionwikis, createcount, editcount, deletecount,
    ...
    font = ImageFont.truetype(
        abspath('./assets/SourceHanSansCN-Normal.ttf'), 40)
    font1 = ImageFont.truetype(
        abspath('./assets/SourceHanSansCN-Normal.ttf'), 70)
    if bantype is None:
    ...
    draww.text(
        ((W - w - font.getoffset(gender)[0]) / 2, 0), gender, "#ffffff", font=font)
    ...
    draww.text(((W - w - font.getoffset(registertime)
               [0]) / 2, 0), registertime, "#ffffff", font=font)
    ...
    draww.text(((W - w - font.getoffset(createcount)
               [0]) / 2, 0), createcount, "#ffffff", font=font)
    ...
    draww.text(((W - w - font.getoffset(editcount)
               [0]) / 2, 0), editcount, "#ffffff", font=font)
    ...
    draww.text(((W - w - font.getoffset(deletecount)
               [0]) / 2, 0), deletecount, "#ffffff", font=font)
    ...
    draww.text(((W - w - font.getoffset(patrolcount)
               [0]) / 2, 0), patrolcount, "#ffffff", font=font)
    ...
    if bantype == 'Y' or bantype == 'YN':
        draw.text((200, 1439), '被' + str(blockbyuser) +
                  '封禁，', '#ffffff', font=font)
        draw.text((200, 1489), '时间从' + str(blocktimestamp1) +
                  '（UTC+8）', '#ffffff', font=font)
        draw.text((200, 1539), '到' + str(blocktimestamp2),
                  '#ffffff', font=font)
    if bantype == 'Y':
