...
async def get_rating(uid, query_type):
    try:
        ...
        font4 = ImageFont.truetype(os.path.abspath(
            './assets/Nunito-Regular.ttf'), 35)
        ...
        drawtext.text((get_img_width - get_name_width - 160, 30),
                      nick, '#ffffff', font=font4)

        font5 = ImageFont.truetype(os.path.abspath(
            './assets/Noto Sans CJK DemiLight.otf'), 20)
        ...
        if __name__ == '__main__':
            ...
            savefilename = os.path.abspath(
                f'{Config("cache_path")}{str(uuid.uuid4())}.jpg')
...
async def make_songcard(workdir, coverpath, chart_type, difficulty, chart_name, score, acc, rt, playtime, rank,
    ...
    drawtext.text((songimg_width - playtime_width - 15, 205),
                  playtime, '#ffffff', font=font3)
    type_ = str(difficulty)
    type_text = Image.new('RGBA', (32, 32))
    draw_typetext = ImageDraw.Draw(type_text)
    draw_typetext.text(
        ((32 - font3.getsize(type_)[0] - font.getoffset(type_)[0]) / 2, 0), type_, "#ffffff", font=font3)
